...
//Declaração de métodos
    ...
    private void atualizarLabel() 
    {
        if (botoesX.isEmpty()) 
        {
            labelQtdCarrinho.setText(null);
        } 
        else if (botoesX.size() == 1) 
        {
            labelQtdCarrinho.setText(botoesX.size() + "");
        } 
        else 
        {
            labelQtdCarrinho.setText(botoesX.size() + "");
...
//Declaração de metodos para adição de componentes que estarao no carrinho 
    ...
    //adiciona a foto do produto
        private JLabel addFotoProduto(JPanel panel) 
        {
            ...
            fotocurso.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/Formas/defaultfotoJanela.png")));
